-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: complete_payment_lifecycle
-- • transition_booking_state: accept accepted+paid → in_progress
-- • complete_booking: create payouts record at completion
-- • create_notification: security-definer helper for cross-user notifications
-- ─────────────────────────────────────────────────────────────────────────────

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. transition_booking_state  – adds accepted+paid → in_progress
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if v_source_status = 'pending_request' and p_target_status = 'accepted' and v_actor_is_cleaner then
    ...
    v_is_allowed := true;
  -- accepted + paid → in_progress (customer has paid, cleaner starts job)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. complete_booking  – adds payouts record at completion
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- Release payment record and stamp fee split; capture payment id for payout
  ...
  -- Create payout record (only if not already present)
  if not exists (select 1 from public.payouts where booking_id = p_booking_id) then
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. create_notification  – security-definer so any role can notify any user
-- ─────────────────────────────────────────────────────────────────────────────
