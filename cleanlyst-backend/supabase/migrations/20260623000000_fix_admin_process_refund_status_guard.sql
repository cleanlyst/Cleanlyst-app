-- =============================================================================
-- FIX: admin_process_refund — booking status update blocked by guard_booking_status_write
-- =============================================================================
-- guard_booking_status_write (20260622300000, updated in 20260622400000) blocks
-- direct writes to bookings.status for the postgres role unless either:
--   a) app.tbs_active = 'true' (set by transition_booking_state)
--   b) app.ledger_sync_active = 'true' (set by sync_booking_from_ledger_event)
--
-- admin_process_refund (20260621300000) updates bookings.status = 'refunded' for
-- full refunds but does not set either flag. The guard blocks the UPDATE, causing
-- the RPC to raise an exception and the UI to show an error instead of success.
--
-- Fix: set app.tbs_active = 'true' locally before the booking status UPDATE.
-- Supersedes admin_process_refund from 20260621300000.
